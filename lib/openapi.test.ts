...
describe("OpenAPI spec", () => {
  ...
  it("premium description marks endpoint public", () => {
    const paths = doc.paths as Record<
      string,
      Record<string, Record<string, unknown>>
    >;
    ...
    assert.ok(description.includes("Public endpoint"));
  });

  it("rates endpoint does not expose server fallback file paths", () => {
    const paths = doc.paths as Record<
      string,
      Record<string, Record<string, unknown>>
    >;
    const rates = paths["/api/rates"].get;
  ...
  it("premium endpoint is public", () => {
    const paths = doc.paths as Record<
      string,
      Record<string, Record<string, unknown>>
    >;
    const premium = paths["/api/premium"].get;
    assert.ok(!premium.security);
    const description = premium.description as string;
    assert.ok(description.includes("Public endpoint"));
  });
});
