...
describe("llms.txt", () => {
  ...
  it("describes premium endpoint as public", () => {
    assert.ok(content.toLowerCase().includes("public"));
  });
...
describe("agents.json", () => {
  ...
  it("has expected capability ids", () => {
    const ids = agents.capabilities.map((c: Record<string, string>) => c.id);
  ...
  it("premium capability is public", () => {
    const premium = agents.capabilities.find(
      (c: Record<string, string>) => c.id === "getPremiumMetrics",
    );
    assert.strictEqual(premium.auth, "none");
  });

  it("rates capability does not expose fallback file paths", () => {
    const rates = agents.capabilities.find(
      (c: Record<string, string>) => c.id === "getRates",
    );
    const parameters = rates.parameters as Array<Record<string, string>>;
  ...
  it("has only public auth scheme", () => {
    assert.ok(agents.auth_schemes);
    assert.ok(agents.auth_schemes.none);
    assert.strictEqual(Object.keys(agents.auth_schemes).length, 1);
  });
});
